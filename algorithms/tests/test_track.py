...
from algorithms.track import (
    predict, search_volume_center_moving, pos3d_in_bounds, angle_acc,
...
from algorithms.parameters import (
    ControlPar, TrackParTuple, read_control_par, read_sequence_par,
    read_track_par, read_volume_par, convert_track_par_to_tuple
)
...
def test_pos3d_in_bounds():
    ...
    bounds = TrackParTuple(
        dvxmin=0.4, dvxmax=120, dvymin=2.0, dvymax=-2.0,
        dvzmin=2.0, dvzmax=-2.0, dangle=2.0, dacc=-2.0,
        add=0, dsumg=0.0, dn=0.0, dnx=0.0, dny=1.0
    )
    ...
    bounds = TrackParTuple(
        dvxmin=0.4, dvxmax=120, dvymin=-2.0, dvymax=2.0, # modified to valid bounds
        dvzmin=-2.0, dvzmax=2.0, dangle=2.0, dacc=-2.0,
        add=0, dsumg=0.0, dn=0.0, dnx=0.0, dny=1.0
    )
...
def test_candsearch_in_pix():
    ...
    test_cpar = ControlPar(num_cams=4, hp_flag=1, all_cam_flag=0, tiff_flag=1, imx=1280, imy=1024, pix_x=0.02, pix_y=0.02, chfield=0)
    test_cpar.mm.n1 = 1
    ...
    p = candsearch_in_pix(test_pix, num_targets, cent_x, cent_y, dl, dr, du, dd, test_cpar)
...
def test_candsearch_in_pix_rest():
    ...
    p = [-999, -999, -999, -999]
    counter = candsearch_in_pix_rest(test_pix, num_targets, cent_x, cent_y, dl, dr, du, dd, p, test_cpar)
...
def test_trackcorr_with_add():
    ...
    try:
        ...
        assert abs(npart - 1.0) < EPS
        assert abs(nlinks - 0.7) < EPS
...
def test_trackback():
    ...
    try:
        ...
        run.tpar = run.tpar._replace(dvxmin=-50, dvymin=-50, dvzmin=-50, dvxmax=50, dvymax=50, dvzmax=50)
        run.lmax = np.linalg.norm([run.tpar.dvxmin - run.tpar.dvxmax,
