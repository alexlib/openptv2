...
pytest.importorskip("flowtracks")

from openptv2.storage import read_zarr_trajectories  # noqa: E402


def _write_linkage(root, frame, prev, pos):
    g = root.require_group(f"linkage/ptv_is/frame_{frame:05d}")
...
def _store(tmp_path):
    return zarr.open_group(str(tmp_path / "run.zarr"), mode="a")
...
def test_stale_trajectories_group_does_not_shadow_linkage(tmp_path):
    ...
    # Fresh tracker output: one particle moving 1 mm/frame over 3 frames.
    for i, frame in enumerate((1, 2, 3)):
        _write_linkage(root, frame, [-1 if i == 0 else 0], [[i * 1.0, 0.0, 0.0]])
    # Stale derived cache from an older, buggy run: 50 mm jumps.
    ...
    trajs = read_zarr_trajectories(tmp_path / "run.zarr")
    assert len(trajs) == 1
    steps = np.linalg.norm(np.diff(np.asarray(trajs[0].pos()) * 1000.0, axis=0), axis=1)
    assert steps.max() == pytest.approx(1.0)  # linkage won, not the 50 mm cache
...
def test_frame_gap_breaks_the_chain(tmp_path):
    ...
    _write_linkage(root, 5, [0], [[900.0, 0, 0]])  # would fake a 900 mm link

    assert read_zarr_trajectories(tmp_path / "run.zarr") == []
