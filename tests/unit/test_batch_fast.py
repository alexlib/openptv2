    ...
    for i, pos in enumerate(positions_3d):
        for cam in range(num_cams):
            x, y = flat_image_coord(
                ...
                cals[cam].ext_par.dm,
                cals[cam].int_par.cc,
                cals[cam].glass_par.vec_x,
                cals[cam].glass_par.vec_y,
...
def test_batch_speedup(setup):
    ...
    bench(
        "pixel_to_metric",
        lambda: [
            pixel_to_metric(xy_pixel[i, 0], xy_pixel[i, 1], cpar) for i in range(N)
        ],
    ...
    bench(
        "metric_to_pixel",
        lambda: [
            metric_to_pixel(xy_metric[i, 0], xy_metric[i, 1], cpar) for i in range(N)
        ],
