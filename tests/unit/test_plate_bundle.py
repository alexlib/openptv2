...
def plate_points():
    ...
    return np.stack(
        [(ix.ravel() - 2) * PITCH, (iy.ravel() - 3) * PITCH, np.zeros(NX * NY)], 1
    ).astype(float)
...
def test_rodrigues_round_trip():
    ...
    for _ in range(20):
        ...
        np.testing.assert_allclose(
            rodrigues(rotvec(rodrigues(r))), rodrigues(r), atol=1e-9
        )
...
def test_tilt_off_vertical():
    ...
    assert tilt_off_vertical_deg(rodrigues([0.0, 0.9, 0.0])) == pytest.approx(
        0.0, abs=1e-9
    )
    # a roll about +Z is not
    assert tilt_off_vertical_deg(
        rodrigues([0.0, 0.0, np.radians(7.0)])
