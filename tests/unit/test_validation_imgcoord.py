...
def load_interpreted_imgcoord():
    ...
    spec = importlib.util.spec_from_file_location(
        "openptv2.algorithms.imgcoord_fallback", str(path)
    )
...
def test_compiled_vs_interpreted_flags(interpreted_module):
    ...
    The primary (imported) module may be compiled (Cython .so) or running as
    pure Python — both modes are valid in this project. The interpreted/fallback
    module loaded via importlib should always report as not compiled since it is
    explicitly loaded as a fresh Python module.
    """
    import openptv2.algorithms.imgcoord as imgcoord_compiled
    ...
    # The primary module reports its true compile state (True if Cython compiled,
    # False when running in pure Python mode).
    # The interpreted fallback always reports False since it's loaded as fresh Python.
...
def test_imgcoord_numerical_parity(interpreted_module):
    ...
    np.testing.assert_allclose(
        py_dist_res_interpreted, c_dist_res, rtol=1e-7, atol=1e-7
    )
    np.testing.assert_allclose(
        py_dist_res_compiled, py_dist_res_interpreted, rtol=1e-7, atol=1e-7
    )
    ...
    py_flat_res_interpreted = interpreted_module.flat_image_coord_batch(
        points, py_cal, py_mm
    )
    ...
    np.testing.assert_allclose(
        py_flat_res_interpreted, c_flat_res, rtol=1e-7, atol=1e-7
    )
    np.testing.assert_allclose(
        py_flat_res_compiled, py_flat_res_interpreted, rtol=1e-7, atol=1e-7
    )
