...
def test_saturating_1d_grows_then_backs_off_at_the_peak():
    ...
    chosen = _saturating_1d(
        ...
        start=0.5, growth=2.0, max_steps=10, patience=2, backoff=1,
    )
...
def test_saturating_1d_backoff_never_goes_below_start():
    ...
    chosen = _saturating_1d(
        eval_fn=lambda v: -v,
        start=1.0, growth=2.0, max_steps=5, patience=1, backoff=5,
    )
    assert chosen == 1.0
...
def test_find_smooth_params_runs_end_to_end_on_synthetic_scenario():
    ...
    spec = bm.ScenarioSpec(
        num_particles=12, num_frames=6, velocity=1.0,
        gap_probability=0.0, noise_mm=0.0, seed=4,
    )
    ...
    result = find_smooth_params(
        yaml_path, "fast_3d",
        dv_start=0.2, dacc_start=0.2, angle_start=10.0,
        growth=1.6, max_steps=6, patience=2, backoff=1,
    )
