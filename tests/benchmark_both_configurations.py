...
def run_configuration_benchmark(title: str, noise: float, gaps: float, spurious: float):
    ...
    print("\n" + "=" * 105)
    print(f"--- {title} ---")
    print(f"    (Particles: {num_particles}, Frames: {num_frames}, Noise std: {noise}, Gaps: {gaps*100:.0f}%, Ghost spurious: {spurious*100:.0f}%)")
    print("=" * 105)
    print(f"{'Tracker Configuration':<36} | {'Yield':<7} | {'Precision':<9} | {'Mean Length':<11} | {'RMS Error':<9} | {'FPS':<8} | {'Throughput':<11}")
    print("-" * 105)
    ...
    m_base = calculate_tracking_metrics(true_tracks, pred_base, distance_tolerance=max(0.2, 3.0 * noise))
    ...
    t0 = time.perf_counter()
    tracker_config_a = MyPTV3DTracker(v_max=3.0, a_max=1.5, max_gap=2, dt=1.0, cost_weights=weights)
    ...
    m_config_a = calculate_tracking_metrics(true_tracks, pred_config_a, distance_tolerance=max(0.2, 3.0 * noise))
    ...
    try:
        ...
        path_prev_arr = [np.full(len(pts), -1, dtype=np.int32) for pts in frame_particle_arrays]
        path_next_arr = [np.full(len(pts), -2, dtype=np.int32) for pts in frame_particle_arrays]
        ...
        m_config_b = calculate_tracking_metrics(true_tracks, pred_config_b, distance_tolerance=max(0.2, 3.0 * noise))
