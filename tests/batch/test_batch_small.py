...
def _read_rt_is(path: Path) -> list[dict]:
    ...
    for line in lines[1 : n + 1]:
        p = line.split()
        rows.append(
            dict(label=int(p[0]), x=float(p[1]), y=float(p[2]), z=float(p[3]),
                 t1=int(p[4]), t2=int(p[5]), t3=int(p[6]), t4=int(p[7]))
        )
    return rows
...
def _read_ptv_is(path: Path) -> list[dict]:
    ...
    for line in lines[1 : n + 1]:
        p = line.split()
        rows.append(
            dict(prev=int(p[0]), next=int(p[1]),
...
def _print_rt_is_summary(frame: int, rows: list[dict]) -> None:
    ...
    if rows:
        ...
        t_counts = [
            sum(1 for r in rows if r[f"t{c}"] >= 0) for c in range(1, 5)
        ]
...
def _print_ptv_is_summary(frame: int, rows: list[dict]) -> None:
    ...
    print(f"    ptv_is.{frame}: {len(rows)} particles  →  {linked} linked  {unlinked} lost")
...
def test_rt_is_positions_vs_ground_truth(small_dir, small_yaml):
    ...
    for frame in FRAMES:
        ...
        rt_pos  = np.array([[r["x"], r["y"], r["z"]] for r in rt_rows]) if rt_rows else np.empty((0, 3))
        ...
        print(f"    matched {matched}/{len(gt_pos)}  recall={recall:.1%}  median_dist={med:.4f} mm")
        if dists:
            print(f"    dist stats: min={min(dists):.4f}  max={max(dists):.4f}  p90={np.percentile(dists,90):.4f}")

        all_dists.extend(dists)
        frame_stats[frame] = dict(matched=matched, total_gt=len(gt_pos), dists=dists, recall=recall)
...
def test_full_tracking_link_counts(small_dir, small_yaml):
    ...
    with tempfile.NamedTemporaryFile("w+", delete=False, suffix=".txt", dir=small_dir) as out_file:
        out_path = out_file.name
        cmd = [
            sys.executable, "-m", "openptv2.batch.pyptv_batch",
            small_yaml.name,
            str(ALL_FRAMES[0]), str(ALL_FRAMES[-1]),
        ]
        print(f"  cmd: {' '.join(cmd)}")
        print(f"  cwd: {small_dir}\n")
        try:
            subprocess.run(
                cmd, stdout=out_file, stderr=subprocess.STDOUT,
                check=True, cwd=small_dir,
            )
...
def test_tracking_trajectories_vs_ground_truth(small_dir, small_yaml):
    ...
    for start_frame in FRAMES:
        for idx, row in enumerate(frame_rows[start_frame]):
            ...
            while True:
                ...
                if nxt < 0:
                    break
                next_frame = FRAMES[FRAMES.index(cur_frame) + 1] if cur_frame in FRAMES[:-1] else None
    ...
    print(f"  full length ({len(FRAMES)} frames): {full}  ({full/len(trajectories):.1%} of reconstructed)")
    print(f"  length distribution  : { {l: lengths.count(l) for l in sorted(set(lengths))} }")
...
def test_single_step_sequence_smoke(small_dir, small_yaml):
    ...
    rows = _read_rt_is(f)
    print(f"  rt_is.10000: {len(rows)} particles  (ground truth has no 3D data for frame 10000, but sequence may still find correspondences)")
