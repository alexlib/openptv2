    ...
    for dacc in acceleration_values:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                if "dacc:" in line and (
                    "track:" in content[: content.find(line)]
                    or i > 0
                    and "track:" in lines[i - 5 : i]
                ):
                    lines[i] = f"  dacc: {dacc}"
                    break
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    if "track:" in line:
                        ...
                        in_track_section and line.strip() and not line.startswith("  ")
                    ):
                        in_track_section = False
...
def run_tracking_test(test_path, test_name):
    ...
    try:
        ...
        tracking_lines = [
            line for line in lines if "step:" in line and "links:" in line
        ]
...
if __name__ == "__main__":
    ...
    print(
        f"Best combination: vel=±{best_combo['vel_range']}, acc={best_combo['dacc']} → {best_combo['link_ratio']:.1f}%"
    )
