...
print(
    f"   Tracked points: {pts_myptv} / {sum(len(f) for f in frames)} ({pts_myptv / sum(len(f) for f in frames) * 100:.1f}%)"
)
...
def fast_cython_enhanced_tracker(frames, v_max=8.0, a_max=25.0):
    ...
    for p in frames[0]:
        active_tracks.append(
            {"id": next_id, "pos": [p], "time": [0], "vel": [np.zeros(3)], "gap": 0}
        )
        next_id += 1
...
print(
    f"   Tracked points: {pts_proto} / {sum(len(f) for f in frames)} ({pts_proto / sum(len(f) for f in frames) * 100:.1f}%)"
)
