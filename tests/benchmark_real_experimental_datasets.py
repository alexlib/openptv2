...
def benchmark_cavity_dataset():
    ...
    try:
        ...
        print("\n" + "=" * 90)
        print(f"--- Real Experimental Dataset Benchmark: TEST_CAVITY ({spar.first} .. {spar.last}) ---")
        print("=" * 90)
        ...
        print(f"{'OpenPTV2 Classic Tracker':<30} | Links: {links_openptv:5d} | Parts: {parts_openptv:5d} | Speed: {fps_openptv:7.1f} FPS")
        ...
        for fn in range(spar.first, spar.last + 1):
            ...
            fr.read("res/rt_is", "", prio_file_base="res/added", target_file_base="", frame_num=fn)
            frames.append(fr)
            frame_particles.append(fr.positions())

        myptv = MyPTV3DTracker(v_max=float(tpar.dvxmax), a_max=float(tpar.dacc), max_gap=1, dt=1.0)
        t0 = time.perf_counter()
        ...
        print(f"{'MyPTV 3D Tracker Baseline':<30} | Links: {myptv_links:5d} | Parts: {parts_openptv:5d} | Speed: {fps_myptv:7.1f} FPS")
        print("=" * 90)
...
def benchmark_burgers_dataset():
    ...
    try:
        ...
        print("\n" + "=" * 90)
        print(f"--- Real Experimental Dataset Benchmark: BURGERS ({spar.first} .. {spar.last}) ---")
        print("=" * 90)
