...
def run_tracking_with_different_parameters():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if links_info:
                        avg_links = sum(info["links"] for info in links_info) / len(
                            links_info
                        )
                        avg_particles = sum(info["curr"] for info in links_info) / len(
                            links_info
                        )
                        link_ratio = (
                            (avg_links / avg_particles * 100)
                            if avg_particles > 0
                            else 0
                        )
    ...
    print("=" * 60)
    print(
        f"{'Parameter Set':<20} {'Link Ratio':<12} {'Avg Links':<10} {'Avg Particles':<12}"
    )
    print("-" * 60)
    ...
    for result in results:
        if result["results"]:
            ...
            print(
                f"{result['name']:<20} {link_ratio:<12.1f}% {avg_links:<10.1f} {avg_particles:<12.1f}"
            )
...
def parse_tracking_output(output_text):
    ...
    for line in tracking_lines:
        try:
            ...
            results.append(
                {"curr": curr_count, "links": links_count, "lost": lost_count}
            )
