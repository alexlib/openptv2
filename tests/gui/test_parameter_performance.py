...
def _run_parameter_access_performance():
    ...
    try:
        ...
        print(
            f"Experiment delegation: {delegation_time:.4f}s ({delegation_time / direct_time:.2f}x slower)"
        )
        print(
            f"Cached access:        {cached_time:.4f}s ({cached_time / direct_time:.2f}x slower)"
        )
        print(
            f"File I/O per reload:  {io_time / 10:.4f}s ({(io_time / 10) / direct_time * 1000:.0f}x slower)"
        )
...
def _run_parameter_change_scenarios():
    ...
    try:
        ...
        if original_n_cam is not None:
            assert restored_n_cam == original_n_cam, (
                f"Failed to restore num_cams: expected {original_n_cam}, got {restored_n_cam}"
            )
        else:
...
if __name__ == "__main__":
    ...
    if perf_results:
        ...
        if perf_results["cached"] < perf_results["direct"] * 0.1:
            print(
                "✓ Caching provides excellent performance - RECOMMENDED for frequently accessed params"
            )
