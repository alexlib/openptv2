...
class TestReadCalibrations:
    ...
    def test_read_calibrations_file_content(self, tmp_path: Path):
        ...
        expected_calls = [
            (
                (
                    str(tmp_path / "cal" / "cam1.ori"),
                    str(tmp_path / "cal" / "cam1.addpar"),
                ),
            ),
            (
                (
                    str(tmp_path / "cal" / "cam2.ori"),
                    str(tmp_path / "cal" / "cam2.addpar"),
                ),
            ),
        ]
        actual_calls = [
            call.args for call in mock_cal_instance.from_file.call_args_list
        ]
...
class TestParameterConsistency:
    ...
    def test_parameter_consistency_n_cam(self):
        ...
        assert len(thresholds) == 4, (
            f"TargetParams always has 4 thresholds, got {len(thresholds)}"
        )
...
class TestCalibrationReadWrite:
    ...
    def test_cal_dir(self):
        ...
        return Path(__file__).parent.parent.parent / "test_data" / "test_cavity" / "cal"
    ...
    def print_calibration_info(self, cal: Calibration, cam_name: str):
        ...
        print(
            f"Camera angles (omega, phi, kappa): {angles[0]:.6f}, {angles[1]:.6f}, {angles[2]:.6f}"
        )

        # Interior orientation
        primary_point = cal.get_primary_point()
        print(
            f"Primary point (xp, yp, c): {primary_point[0]:.6f}, {primary_point[1]:.6f}, {primary_point[2]:.6f}"
        )

        # Radial distortion
        radial_dist = cal.get_radial_distortion()
        print(
            f"Radial distortion (k1, k2, k3): {radial_dist[0]:.6f}, {radial_dist[1]:.6f}, {radial_dist[2]:.6f}"
        )
        ...
        print(
            f"Glass vector: {glass_vec[0]:.6f}, {glass_vec[1]:.6f}, {glass_vec[2]:.6f}"
        )
    ...
    def test_read_real_calibration_files(self, capsys):
        ...
        for i, cal in enumerate(calibrations):
            ...
            assert not np.allclose(pos, [0, 0, 0]), (
                f"Camera {i + 1} has invalid position"
            )
    ...
    def test_calibration_round_trip_filecmp(self):
        ...
        try:
            for cam_file in cam_files:
                ...
                input_ori_file = str(self.test_cal_dir / f"{cam_file}.ori").encode(
                    "utf-8"
                )
                input_add_file = str(self.test_cal_dir / f"{cam_file}.addpar").encode(
                    "utf-8"
                )
                output_ori_file = str(
                    self.output_directory / f"output_{cam_file}.ori"
                ).encode("utf-8")
                output_add_file = str(
                    self.output_directory / f"output_{cam_file}.addpar"
                ).encode("utf-8")
                ...
                np.testing.assert_array_almost_equal(
                    orig_cal.get_pos(), copied_cal.get_pos(), decimal=10
                )
                np.testing.assert_array_almost_equal(
                    orig_cal.get_angles(), copied_cal.get_angles(), decimal=10
                )
                np.testing.assert_array_almost_equal(
                    orig_cal.get_primary_point(),
                    copied_cal.get_primary_point(),
                    decimal=10,
                )
                np.testing.assert_array_almost_equal(
                    orig_cal.get_radial_distortion(),
                    copied_cal.get_radial_distortion(),
                    decimal=10,
                )
                np.testing.assert_array_almost_equal(
                    orig_cal.get_decentering(), copied_cal.get_decentering(), decimal=10
                )
                np.testing.assert_array_almost_equal(
                    orig_cal.get_affine(), copied_cal.get_affine(), decimal=10
                )
                np.testing.assert_array_almost_equal(
    ...
    def test_calibration_parameter_setters(self):
        ...
        try:
            ...
            assert not np.array_equal(dmatrix_before, dmatrix_after), (
                "Rotation matrix should change"
            )
    ...
    def test_file_content_comparison(self, tmp_path: Path):
        ...
        for cam_file in cam_files:
            ...
            cal.from_file(
                (orig_cal_base + ".ori").encode("utf-8"),
                (orig_cal_base + ".addpar").encode("utf-8"),
            )
            ...
            cal.write(
                (copy_cal_base + ".ori").encode("utf-8"),
                (copy_cal_base + ".addpar").encode("utf-8"),
            )
        ...
        try:
            ...
            for i, cal in enumerate(cals):
                assert isinstance(cal, Calibration), (
                    f"Camera {i + 1} is not a Calibration object"
                )
                ...
                focal = cal.get_primary_point()[
                    2
                ]  # Focal length is 3rd element of primary point
    ...
    def test_modified_calibration_write(self, tmp_path: Path):
        ...
        new_primary_point = np.array(
            [orig_primary_point[0], orig_primary_point[1], new_focal]
        )
