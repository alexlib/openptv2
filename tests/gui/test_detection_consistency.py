...
from openptv2.gui.pyptv.ptv import py_detection_proc_c, py_start_proc_c, _populate_tpar
...
class TestDetectionConsistency:
    ...
    def test_tpar_parameter_consistency(self, experiment):
        ...
        np.testing.assert_array_equal(
            tpar.get_grey_thresholds(), tpar_gui.get_grey_thresholds()
        )
    ...
    def test_detection_consistency(self, experiment):
        ...
        # Compare the TargetParams objects - they should be identical
        np.testing.assert_array_equal(
            tpar_seq.get_grey_thresholds(), tpar_gui.get_grey_thresholds()
        )
    ...
    def test_parameter_sections_exist(self, experiment):
        ...
        print(
            f"detect_plate grey thresholds: [gvth_1={detect_plate.get('gvth_1')}, gvth_2={detect_plate.get('gvth_2')}, gvth_3={detect_plate.get('gvth_3')}, gvth_4={detect_plate.get('gvth_4')}]"
        )

        print(
            f"targ_rec pixel bounds: nnmin={targ_rec.get('nnmin')}, nnmax={targ_rec.get('nnmax')}"
        )
        print(
            f"detect_plate pixel bounds: min_npix={detect_plate.get('min_npix')}, max_npix={detect_plate.get('max_npix')}"
        )
