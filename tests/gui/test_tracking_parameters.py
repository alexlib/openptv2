...
def test_tracking_parameters_propagation():
    ...
    assert isinstance(track_params_yaml, dict), (
        f"Track parameters should be dict, got {type(track_params_yaml)}"
    )
    ...
    print(
        f"Testing tracking with velocity ranges: x={track_par.get_dvxmin()}-{track_par.get_dvxmax()}, "
