    ...
    try:
        ...
        if len(targets) > 0:
            for i in range(min(3, len(targets))):
                t = targets[i]
                print(
                    f"   Target {i}: pos=({t.pos()[0]:.1f}, {t.pos()[1]:.1f}), "
...
def test_coordinate_transforms():
    ...
    pixel_coords = np.array(
        [[100.0, 100.0], [200.0, 200.0], [300.0, 300.0]], dtype=np.float64
    )
...
def main():
    ...
    print("=" * 60)
    print(
        f"optv version: {__import__('optv').__version__ if 'optv' in sys.modules or importlib.util.find_spec('optv') else 'N/A'}"
    )
