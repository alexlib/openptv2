...
class TestTrackingParameterBug:
    ...
    def _run_cavity_tracking_parameter_translation(self):
        ...
        try:
            ...
            for yaml_key, cython_key in yaml_to_cython_mapping.items():
                ...
                if (
                    abs(yaml_val - cython_val) > 1e-6
                ):  # Allow for small floating point differences
                    mismatches.append(
                        f"{yaml_key}: YAML={yaml_val} vs Cython={cython_val}"
                    )
                    print(
                        f"  MISMATCH {yaml_key}: YAML={yaml_val} vs Cython={cython_val}"
                    )
                else:
            ...
            print(
                f"  Velocity range X: {vel_range_x} (min: {translated_params['dvxmin']}, max: {translated_params['dvxmax']})"
            )
            print(
                f"  Velocity range Y: {vel_range_y} (min: {translated_params['dvymin']}, max: {translated_params['dvymax']})"
            )
            print(
                f"  Velocity range Z: {vel_range_z} (min: {translated_params['dvzmin']}, max: {translated_params['dvzmax']})"
            )
            ...
            if angle_val > 180:
                warnings.append(
                    f"Very large angle parameter: {angle_val} (typical values are 0-180)"
                )
    ...
    def test_parameter_comparison(self):
        ...
        print("\n=== COMPARISON COMPLETE ===")
        print(
            "Review the output above to identify differences between test_cavity and test_splitter"
        )
        print("Look for:")
