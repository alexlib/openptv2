...
def mock_info():
    ...
    obj.get_parameter = MagicMock(
        return_value={"first": 1, "last": 2, "base_name": ["base1", "base2"]}
    )
    obj.cals = ["cal1", "cal2"]
    obj.cpar = MagicMock()
...
def patch_flowtracks_and_helpers(mock_traj):
    with patch(
        "openptv2.gui.flowtracks_utils.compute_flowtracks_trajectories_from_guiobj"
    ) as mock_compute:
...
def test_traject_action_flowtracks_calls_and_draws(
