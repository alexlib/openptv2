...
def test_cavity_directory_structure():
    ...
    assert test_cavity_path.exists(), (
        f"Test cavity directory does not exist: {test_cavity_path}"
    )
...
def test_experiment_initialization(test_cavity_setup):
    ...
    assert experiment.pm.num_cams == 4, (
        f"Expected 4 cameras, got {experiment.pm.num_cams}"
    )
...
def test_parameter_loading(test_cavity_setup):
    ...
    assert ptv_params.get("imx") == 1280, (
        f"Expected image width 1280, got {ptv_params.get('imx')}"
    )
    assert ptv_params.get("imy") == 1024, (
        f"Expected image height 1024, got {ptv_params.get('imy')}"
    )
    ...
    assert len(base_names) >= 4, (
        f"Expected at least 4 base names, got {len(base_names)}"
    )

    expected_names = ["img/cam1.%d", "img/cam2.%d", "img/cam3.%d", "img/cam4.%d"]
    for i, expected in enumerate(expected_names):
        assert base_names[i] == expected, (
            f"Base name mismatch: expected {expected}, got {base_names[i]}"
        )
...
def test_parameter_manager_debugging(test_cavity_setup):
    ...
    print(
        f"ParameterManager methods: {[m for m in dir(experiment.pm) if not m.startswith('_')]}"
    )
...
def test_image_files_exist(test_cavity_setup):
    ...
    for i, base_name in enumerate(base_names[:num_cams]):
        ...
        assert img.min() >= 0 and img.max() <= 255, (
            f"Image values out of range: {img.min()}-{img.max()}"
        )
    ...
    assert len(loaded_images) == num_cams, (
        f"Expected {num_cams} images, loaded {len(loaded_images)}"
    )
...
def test_pyptv_core_initialization(test_cavity_setup):
    ...
    try:
        ...
        assert len(cals) == experiment.pm.num_cams, (
...
def test_image_preprocessing(test_cavity_setup):
    ...
    assert len(processed_images) == len(orig_images), (
        "Preprocessing changed number of images"
    )
    for i, (orig, proc) in enumerate(zip(orig_images, processed_images)):
        ...
        print(
            f"Image {i}: original range {orig.min()}-{orig.max()}, processed range {proc.min()}-{proc.max()}"
        )
...
def test_particle_detection(test_cavity_setup):
    ...
    try:
        ...
        assert total_detections > 0, (
            "No particles detected - check detection parameters or image quality"
        )
