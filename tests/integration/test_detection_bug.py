...
def test_detection_parameters_bug():
    ...
    test_dir = Path("tests/integration/test_cavity")
    ...
    if img_path.exists():
        ...
        if target_params_seq is not None:
            try:
                print(
                    "   Testing sequence detection (detect_plate - proper parameters):"
                )
    ...
    print(
        "This explains why sequence processing gets more detections than manual GUI steps."
    )
