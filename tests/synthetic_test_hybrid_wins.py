...
print(
    f"  2D Image Separation on Cam 1: {p2d_sep:.2f} pixels (Overlapping Sensor Spots!)"
)
...
print("1. fast_3d Alone:")
print(f"   Tracked Links: {links_fast3d} / 3 particles ({links_fast3d / 3 * 100:.1f}%)")
print(
    "   Reason: P1 and P2 tracked cleanly in 3D, but P3 missed due to lack of f0 seed."
)
...
print("\n2. trackcorr Alone:")
print(
    f"   Tracked Links: {links_trackcorr} / 3 particles ({links_trackcorr / 3 * 100:.1f}%)"
)
print(
    "   Reason: P3 discovered by 2D re-triangulation, but P2 dropped due to 2D Cam 1 target overlap."
)
...
print(
    f"   Tracked Links: {links_hybrid} / 3 particles ({links_hybrid / 3 * 100:.1f}% PERFECT RECONSTRUCTION!)"
)
print("   Reason: Pass 1 (fast_3d) resolves 3D spatial overlap for P1 & P2;")
print(
    "           Pass 2 (trackcorr) re-triangulates unmatched 2D target peaks to discover P3!"
)
