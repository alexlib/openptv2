...
def generate_cavity_assets():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        rcm_red = (
            (1.0 - info["rcm_after"] / info["rcm_before"]) * 100.0
            if info.get("rcm_before")
            else 0.0
        )
        print(
            f"Shaking RCM: {info['rcm_before']:.2f} um -> {info['rcm_after']:.2f} um ({rcm_red:.1f}% reduction)"
        )
        ...
        print(
            "Tuned kinematic parameters: search box = [-1.0, 1.0] mm, dacc = 1.0 mm, dangle = 120 deg"
        )
        ...
        if store is not None:
            for frame_idx, frame_num in enumerate(range(first, last + 1)):
                ...
                point_by_frame[frame_num] = {"pos": pos, "links": links}
                print(
                    f"Frame {frame_num}: {len(pos)} particles, {np.sum(links >= 0)} forward links"
                )
        else:
            for frame_idx, frame_num in enumerate(range(first, last + 1)):
                ...
                if ptv_path.exists():
                    ...
                    if len(data) > 0:
                        ...
                        print(
                            f"Frame {frame_num}: {len(pos)} particles, {np.sum(next_l >= 0)} forward links"
                        )
        ...
        for start_idx, start_pos in enumerate(point_by_frame[first]["pos"]):
            ...
            while curr_link >= 0 and curr_f <= last:
                if curr_f in point_by_frame and curr_link < len(
                    point_by_frame[curr_f]["pos"]
                ):
        ...
        norm = plt.Normalize(
            vmin=np.percentile(vel_mags, 5), vmax=np.percentile(vel_mags, 95)
        )
        cmap = plt.cm.viridis
        ...
        for traj, v in zip(trajectories, vel_mags):
            ...
            ax.scatter(
                traj[-1, 0], traj[-1, 1], traj[-1, 2], color=color, s=8, alpha=0.8
            )
        ...
        ax.set_title(
            f"OpenPTV2: Cavity Flow 3D Trajectories ({len(trajectories)} Tracks)",
            fontsize=13,
            fontweight="bold",
            pad=15,
        )
        ...
        cbar.set_label(
            "Mean Velocity Magnitude [mm/frame]", fontsize=10, fontweight="bold"
        )
        ...
        for cam_i, ax in enumerate(axes):
            ...
            if img_path.exists():
                try:
                    ...
                    ax.imshow(
                        np.zeros((cpar.imy, cpar.imx)), cmap="gray", origin="upper"
                    )
            else:
            ...
            ax.set_title(
                f"Camera {cam_num} (Frame {frame_num}) - {len(targs)} Detections",
                fontsize=11,
                fontweight="bold",
            )
        ...
        plt.suptitle(
            "Multi-Camera 2D Target Detections vs 3D Stereo Reprojections",
            fontsize=14,
            fontweight="bold",
            y=0.98,
        )
        ...
        for traj, v in zip(trajectories, vel_mags):
            ...
            ax.scatter(
                traj[-1, 0], traj[-1, 1], traj[-1, 2], color=color, s=6, alpha=0.85
            )
        ...
        ax.set_title(
            "OpenPTV2 Cavity Flow - 3D Trajectories", fontsize=11, fontweight="bold"
        )
        ax.grid(True, linestyle="--", alpha=0.3)
        ...
        shutil.copyfile(
            reproj_png, docs_img_dir / "test_cavity_multicam_reprojection.png"
        )
