...
def _(mo):
    ...
    marker_size_slider = mo.ui.slider(
        start=1,
        stop=15,
        step=1,
        value=6,
...
def _(data, load_error, mo, np):
    if load_error or len(data) == 0:
        stats_view = mo.md(
            f"⚠️ **Error loading file:** {load_error or 'No data found'}"
        )
...
def _(
    ...
    if len(data) == 0:
        ...
        hover_text = [
            (
                f"<b>Point ID:</b> {pid}<br><b>X:</b> {px:.1f} mm"
                f"<br><b>Y:</b> {py:.1f} mm<br><b>Z:</b> {pz:.1f} mm"
            )
