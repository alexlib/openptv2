...
def _(num_cams, pm, yaml_path):
    ...
    for i in range(num_cams):
        ...
        try:
            ...
            images.append(
                np.zeros((ptv_params.get("imy", 1024), ptv_params.get("imx", 1024)))
            )
        ...
        if ori_file_path.exists() and addpar_file_path.exists():
            ...
            print(
                f"Missing calibration files for camera {i + 1}: {ori_file_path} / {addpar_file_path}"
            )
...
def _(cals, cpar, images_8bit, tpar, vpar):
    ...
    sorted_pos, sorted_corresp, num_targs = correspondences(
        targets, matched, cals, vpar, cpar
    )
        ...
        for j_other_corr in range(num_cams):
            ...
            try:
                ...
                if len(pts_epipolar_corr) > 1:
                    ...
                    axes_flat_corr[j_other_corr].plot(
                        pts_epipolar_corr[:, 0],
                        pts_epipolar_corr[:, 1],
                        color=colors_corr[clicked_i_corr],
                        linewidth=1.5,
                    )
            except Exception as e:
...
def _(cals, cpar, matched, sorted_corresp, sorted_pos, vpar):
    ...
    flat = np.array(
        [
            ...
            for corr, corresp in zip(matched, concatenated_corresp)
        ]
    )
    ...
    class Camera_Calibration_API:
        ...
        def __init__(
            ...
            assert pattern_type in pattern_types, (
                "pattern type must be one of {}".format(pattern_types)
            )
            ...
            if self.pattern_type in ["asymmetric_circles", "symmetric_circles"]:
                ...
                self.blobParams.minArea = (
                    2  # minArea may be adjusted to suit for your experiment
                )
        ...
        def _symmetric_world_points(self):
            ...
            pattern_points = np.hstack(
                (x.reshape(prod, 1), y.reshape(prod, 1), np.zeros((prod, 1)))
            ).astype(np.float32)
        ...
        def _asymmetric_world_points(self):
            ...
            pattern_points = np.hstack(
                (
                    pattern_points,
                    np.zeros((self.pattern_rows * self.pattern_columns, 1)),
                )
            ).astype(np.float32)
            return pattern_points
        ...
        def _calc_reprojection_error(self, figure_size=(8, 8), save_dir=None):
            ...
            for i in range(len(self.calibration_df)):
                imgpoints2, _ = cv2.projectPoints(
                    self.calibration_df.obj_points[i],
                    self.calibration_df.rvecs[i],
                    self.calibration_df.tvecs[i],
                    self.camera_matrix,
                    self.dist_coefs,
                )
                temp_error = cv2.norm(
                    self.calibration_df.img_points[i], imgpoints2, cv2.NORM_L2
                ) / len(imgpoints2)
                reprojection_error.append(temp_error)
            self.calibration_df["reprojection_error"] = pd.Series(reprojection_error)
            avg_error = np.sum(np.array(reprojection_error)) / len(
                self.calibration_df.obj_points
            )
            ...
            # Plot the data
            ax.scatter(
                x, reprojection_error, label="Reprojection error", marker="o"
            )  # plot before
        ...
        def calibrate_camera(
            ...
            * custom_image_points_function(img,pattern_rows,pattern_columns):
            ...
            if self.pattern_type == "custom":
                assert custom_world_points_function is not None, (
                    "Must implement a custom_world_points_function for 'custom' pattern "
                )
                assert custom_image_points_function is not None, (
                    "Must implement a custom_image_points_function for 'custom' pattern"
                )
            ...
            if origin_image_filename is not None:
                images_path_list = [
                    img for img in images_path_list if img != origin_image_filename
                ]
                images_path_list.insert(0, origin_image_filename)
            images_path_list.sort()
            print(
                "There are {} {} images given for calibration".format(
                    len(images_path_list), self.pattern_type
                )
            )

            if self.pattern_type == "chessboard":
                pattern_points = (
                    self._symmetric_world_points() * self.distance_in_world_units
                )
                ...
                pattern_points = (
                    self._asymmetric_world_points() * self.distance_in_world_units
                )
                if self.blobDetector is None:
                    blobDetector = cv2.SimpleBlobDetector_create(self.blobParams)
                else:
                ...
                pattern_points = custom_world_points_function(
            ...
            def process_single_image(img_path):
                ...
                assert w == img.shape[1] and h == img.shape[0], (
                    "All the images must have same shape"
                )

                if self.pattern_type == "chessboard":
                    found, corners = self._chessboard_image_points(img)
                elif (
                    self.pattern_type == "asymmetric_circles"
                    or self.pattern_type == "symmetric_circles"
                ):
                    found, corners = self._circulargrid_image_points(
                        img, flags, blobDetector
                    )

                elif self.pattern_type == "custom":
                    found, corners = custom_image_points_function(
                        img, self.pattern_rows, self.pattern_columns
                    )
                    assert corners[0] == pattern_points[0], (
                        "custom_image_points_function should return a numpy array of length matching the number of control points in the image"
                    )
                ...
                if found:
                    ...
                    if self.subpixel_refinement:
                        corners2 = cv2.cornerSubPix(
                            img, corners, (11, 11), (-1, -1), self.term_criteria
                        )
                    else:
            ...
            if threads_num <= 1:
                calibrationBoards = [
                    process_single_image(img_path) for img_path in images_path_list
                ]
            else:
            ...
            self.rms, self.camera_matrix, self.dist_coefs, rvecs, tvecs = (
                cv2.calibrateCamera(
                    self.calibration_df.obj_points,
            ...
            self._calc_reprojection_error(
                figure_size=self.figsize, save_dir=self.debug_dir
            )
        ...
        def visualize_calibration_boards(
            self, cam_width=20.0, cam_height=10.0, scale_focal=40
        ):
            """
...
def _(cm, cv2, linspace):
    def _draw_camera_boards(
        ...
        if patternCentric:
            X_moving = _create_camera_model(
                camera_matrix, cam_width, cam_height, scale_focal
            )
            X_static = _create_board_model(
                extrinsics, board_width, board_height, square_size
            )
        else:
            X_static = _create_camera_model(
                camera_matrix, cam_width, cam_height, scale_focal, True
            )
            X_moving = _create_board_model(
                extrinsics, board_width, board_height, square_size
            )
        ...
        for idx in range(extrinsics.shape[0]):
            ...
            for i in range(len(X_moving)):
                ...
                for j in range(X_moving[i].shape[1]):
                    X[0:4, j] = _transform_to_matplotlib_frame(
...
def visualize_views(
    ...
    min_values, max_values = _draw_camera_boards(
        ax,
        camera_matrix,
        cam_width,
        cam_height,
