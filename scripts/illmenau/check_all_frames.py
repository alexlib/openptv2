...
def plane_and_pitch(pos, ids):
    ...
    if not dx or not dy:
        return None
    return (float(np.sqrt(np.mean(resid ** 2))), float(np.abs(resid).max()),
            float(np.median(dx)), float(np.median(dy)), float(np.linalg.norm(c)))


print("                 ALL dots                |        RCM-inlier dots (rays converge < "
...
for fr in sorted({f for _, f in views}):
    ...
    keep = rcm < RCM_TOL
    inl = (plane_and_pitch(pos[keep], [p for p, k in zip(ids, keep) if k])
           if keep.sum() >= 8 else None)
    ...
    print(f"{fr}  {len(ids):3d}  {allf[0]:8.3f} {allf[2]:7.2f} {allf[3]:7.2f}  | "
...
print(f"inlier planarity RMS : median {np.median(rms_in):.3f} mm, p90 "
      f"{np.percentile(rms_in, 90):.3f} mm, max {rms_in.max():.3f} mm")
print(f"inlier pitch         : X {np.median(px_in):.3f} mm ({100*(np.median(px_in)/PITCH-1):+.2f} %), "
...
for lo, hi in zip(edges[:-1], edges[1:]):
    ...
    if not m.any():
        continue
    print(f"  {lo:5.0f} - {hi if hi < 1e8 else 99999:5.0f} mm       {m.sum():4d}   {int(n_in[m].sum()):4d}"
