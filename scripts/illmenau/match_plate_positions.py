...
def ideal_grid(ids):
    ...
    return np.stack(
        [(ix - DATUM[0]) * PITCH, (iy - DATUM[1]) * PITCH, np.zeros(len(ids))], 1
    ).astype(float)
...
def datum_positions(folder, cams):
    ...
    for n in cams:
        c = Calibration()
        c.from_file(
            str(folder / f"cal/cam{n}.tif.ori"), str(folder / f"cal/cam{n}.tif.addpar")
        )
        cals.append(c)
...
print(
    f"signature matching: {len(ri)} candidate pairs, cost median "
...
for it in range(5):
    ...
    print(
        f"  round {it}: {keep.sum():3d} pairs, residual median "
...
print(
    f"  residual over accepted pairs: median {np.median(err[keep]):.2f} mm, "
...
print(
    f"  along  the normal: median {np.median(along):6.2f} mm   "
    f"[{along.min():.2f} .. {along.max():.2f}]"
)
print(
    f"  across the normal: median {np.median(across):6.2f} mm   "
...
with dst.open("w") as fh:
    fh.write(
        "front_frame,back_frame,accepted,residual_mm,along_normal_mm,"
