...
SWEEP = {
    ...
    "calib_severity": dict(calib_angle_sigma_deg=0.04, calib_pos_sigma_mm=0.04, calib_cc_ppm=400.0),
}
...
def run_one(label: str, params: dict) -> None:
    ...
    print(f"{'tracker':<22} {'a_err':>8} {'K_a':>8} {'>5sig':>8} {'meanlen':>8} {'yield':>7}")
    for tr in TRACKERS:
        try:
            pred0, _dt = bu.run_single_tracker(tr, track_overrides=overrides[tr], src=SRC, first=FIRST)
        ...
        print(f"{tr:<22} {100*(a_rms/a_rms_t-1):+7.1f}% {a_k:8.2f} {outl:7.3f}% "
              f"{lens.mean():8.2f} {m['yield_recall']:7.4f}", flush=True)
