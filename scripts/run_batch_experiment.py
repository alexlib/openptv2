...
def verify_folder_setup(exp_root: Path, run_cfg: dict, shared_cal_dir: str):
    ...
    report.append(
        f"Found {len(tifs)} raw .tif images ({tifs[0].name} ... {tifs[-1].name})"
    )
    ...
    for label, y_path in [
        ("Sample YAML", sample_yaml_path),
        ("Batch YAML", batch_yaml_path),
    ]:
        ...
        if not fixp.exists():
            report.append(
                f"Calibration fixp '{fixp}' referenced in {y_path.name} DOES NOT EXIST!"
            )
            is_valid = False
        ...
        for ori_rel in y_data["cal_ori"]["img_ori"]:
            ...
            if not ori_p.exists():
                report.append(
                    f"Camera orientation file '{ori_p}' referenced in {y_path.name} DOES NOT EXIST!"
                )
                is_valid = False
        ...
        if not first_frame_path.exists():
            report.append(
                f"First frame {seq_first} ({first_frame_path.name}) specified in {y_path.name} NOT FOUND on disk!"
            )
            is_valid = False

        if not last_frame_path.exists():
            report.append(
                f"Last frame {seq_last} ({last_frame_path.name}) specified in {y_path.name} NOT FOUND on disk!"
            )
            is_valid = False

        report.append(
            f"Verified {y_path.name}: sequence template '{base_tmpl}', range {seq_first}..{seq_last}"
        )

    return is_valid, report, info


def run_and_monitor_openptv2_batch(
    yaml_path: Path, log_file: Path, anomaly_cfg: dict, exp_root: Path
):
    ...
    with open(log_file, "w", encoding="utf-8") as lf:
        lf.write(
            f"=== OpenPTV2 Monitored Batch Started at {datetime.datetime.now()} ===\n"
        )
        ...
        for line in proc.stdout:
            ...
            if folder_abort_file.exists() or root_abort_file.exists():
                log_print(
                    f"\n{Term.FAIL}[ABORT TRIGGER] Detected abort signal file ({abort_trigger_name})! Stopping run...{Term.ENDC}",
                    [log_file],
                )
                proc.terminate()
                anomalies.append(
                    {"frame": -1, "message": "Manual abort trigger activated"}
                )
                break
            ...
            if "Running sequence plugin" in line:
                ...
                "Running tracking plugin" in line
                or "Running Standard Forward Tracking" in line
            ):
            ...
            if m_corr:
                ...
                moving_avg = (
                    sum(recent_particles[-10:]) / len(recent_particles[-10:])
                    if recent_particles
                    else float(total_parts)
                )
                ...
                if is_anomaly:
                    ...
                    log_print(
                        f"  {Term.WARNING}[ANOMALY ALERT @ Frame {frame_num}] {anomaly_msg}{Term.ENDC}",
                        [log_file],
                    )
                    if stop_on_anomaly:
                        log_print(
                            f"{Term.FAIL}[CIRCUIT BREAKER] Aborting run due to severe anomaly.{Term.ENDC}",
                            [log_file],
                        )
                        proc.terminate()
                        break
                ...
                if (
                    ...
                    log_print(
                        f"  [{current_stage}] Frame {frame_num} ({frames_done}/{total_frames} | {pct:.1f}%) | Particles: {total_parts} (avg: {moving_avg:.1f}) | {fps:.1f} fps | ETA: {eta_sec:.0f}s",
                        [log_file],
                        color=Term.OKCYAN,
                    )
            ...
            if m_step:
                ...
                if step_num % 10 == 0 or step_num == 1:
                    log_print(
                        f"  [3D_TRACKING Step {step_num}] Particles: {curr_p} -> Active Links: {links}",
                        [log_file],
                        color=Term.OKBLUE,
                    )
    ...
    final_status = (
        "SUCCESS"
        if proc.returncode == 0 and not anomalies
        else ("ABORTED" if anomalies and stop_on_anomaly else "COMPLETED_WITH_WARNINGS")
    )
    prog_data = {
        "stage": "COMPLETED",
        "progress_pct": 100.0,
...
def parse_batch_output(output_lines):
    ...
    if detected_particles:
        metrics["avg_particles"] = float(
            sum(detected_particles) / len(detected_particles)
        )
    if step_links:
    ...
    if (
        ...
        and metrics["avg_links"] > 0
        and len(metrics["correspondences"]) > 0
    ):
        metrics["total_links"] = int(
            metrics["avg_links"] * (len(metrics["correspondences"]) - 1)
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="OpenPTV2 Monitored Batch Processing Runner"
    )
    parser.add_argument(
        "--config", default="batch_config.yaml", help="Path to batch_config.yaml"
    )
    parser.add_argument(
        "--sample-only", action="store_true", help="Run step 1 (sample) only"
    )
    parser.add_argument(
        "--skip-sample",
        action="store_true",
        help="Skip step 1 and run full batch directly",
    )
    ...
    master_log = (
        log_dir
        / f"master_batch_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    )
    ...
    log_print(
        f"  OpenPTV2 Monitored Batch Processing - Experiment: {exp_name}",
    ...
    log_print(
        ">>> Step 0: Verifying internal folder configurations & disk images...",
        log_files=[master_log],
        color=Term.OKCYAN,
    )
    all_verified = True
    ...
    for run in runs:
        log_print(
            f"\n[Run: {run['name']}] Checking folder '{run['folder']}'...",
            log_files=[master_log],
        )
        ok, report_lines, info = verify_folder_setup(
            exp_root, run, cfg.get("shared_calibration_dir", "")
        )
    ...
    if not all_verified:
        log_print(
            f"\n{Term.FAIL}Pre-flight verification failed! Please fix errors listed above before running.{Term.ENDC}",
            log_files=[master_log],
        )
        sys.exit(1)
    ...
    if not args.skip_sample:
        ...
        log_print(
            "  Step 1: Running Small Subset / Sample Check",
            log_files=[master_log],
            color=Term.BOLD,
        )
        log_print(
        ...
        for run in runs:
            ...
            log_print(
                f"\n[{r_name}] Launching sample run: {sample_yaml.name} ...",
                log_files=[master_log, run_log],
                color=Term.OKBLUE,
            )

            t0 = time.time()
            ret_code, out_lines = run_and_monitor_openptv2_batch(
                sample_yaml, run_log, anomaly_cfg, exp_root
            )
            elapsed = time.time() - t0

            with (
                open(run_log, "r", encoding="utf-8") as rf,
                open(per_folder_log, "w", encoding="utf-8") as pf,
            ):
                pf.write(rf.read())

            if ret_code != 0:
                log_print(
                    f"[{r_name}] {Term.FAIL}Sample run FAILED with exit code {ret_code}! See log: {run_log}{Term.ENDC}",
                    log_files=[master_log, run_log],
                )
                sample_passed = False
                continue
            ...
            log_print(
                f"[{r_name}] Sample run completed in {elapsed:.2f}s ({n_frames} frames)",
                log_files=[master_log, run_log],
            )
            log_print(
                f"  - Avg particles/frame: {avg_m:.1f}", log_files=[master_log, run_log]
            )
            log_print(
                f"  - Avg links/frame: {metrics['avg_links']:.1f}",
                log_files=[master_log, run_log],
            )
            ...
            if avg_m < min_matches or tot_l < min_tracks or not metrics["completed"]:
                log_print(
                    f"  - {Term.FAIL}QUALITY GATE FAILED for {r_name}:{Term.ENDC} (matches={avg_m:.1f} < {min_matches} or links={tot_l} < {min_tracks})",
                    log_files=[master_log, run_log],
                )
                sample_passed = False
            else:
                log_print(
                    f"  - {Term.OKGREEN}QUALITY GATE PASSED for {r_name}!{Term.ENDC}",
                    log_files=[master_log, run_log],
                )

        if args.sample_only:
            log_print(
                f"\n{Term.OKCYAN}Sample run completed (--sample-only specified). Stopping here.{Term.ENDC}",
                log_files=[master_log],
            )
            return
        ...
        if not sample_passed:
            log_print(
                f"\n{Term.FAIL}!!! Quality Gate FAILED for one or more sample runs. Aborting full batch processing. !!!{Term.ENDC}",
                log_files=[master_log],
            )
            sys.exit(1)
    ...
    log_print(
        "  Step 2: Executing Full Monitored Batch Run",
        log_files=[master_log],
        color=Term.BOLD,
    )
    log_print(
    ...
    for run in runs:
        ...
        log_print(
            f"\n[{r_name}] Launching full batch run: {batch_yaml.name} ...",
            log_files=[master_log, run_log],
            color=Term.OKBLUE,
        )
        ...
        t0 = time.time()
        ret_code, out_lines = run_and_monitor_openptv2_batch(
            batch_yaml, run_log, anomaly_cfg, exp_root
        )
        elapsed = time.time() - t0
        ...
        if ret_code != 0:
            log_print(
                f"[{r_name}] {Term.FAIL}Full batch run FAILED with exit code {ret_code}! See log: {run_log}{Term.ENDC}",
                log_files=[master_log, run_log],
            )
        ...
        log_print(
            f"[{r_name}] {Term.OKGREEN}Full batch completed successfully in {elapsed:.2f}s ({elapsed / 60.0:.2f} min){Term.ENDC}",
            log_files=[master_log, run_log],
        )
        log_print(
            f"  - Frames processed: {len(metrics['correspondences'])}",
            log_files=[master_log, run_log],
        )
        log_print(
            f"  - Avg particles/frame: {metrics['avg_particles']:.1f}",
            log_files=[master_log, run_log],
        )
        log_print(
            f"  - Total links: {metrics['total_links']}",
            log_files=[master_log, run_log],
        )
    ...
    log_print(
        "  Final Multi-Folder Batch Execution Summary",
        log_files=[master_log],
        color=Term.BOLD,
    )
    log_print(
    ...
    log_print(
        f"{'Run Folder':<12} | {'Status':<10} | {'Frames':<8} | {'Avg Particles':<14} | {'Total Links':<12} | {'Time (min)':<10}",
        log_files=[master_log],
    )
    ...
    for r_name, res in batch_summary.items():
        if res["success"]:
            log_print(
                f"{r_name:<12} | SUCCESS    | {res['frames']:<8} | {res['avg_particles']:<14.1f} | {res['total_links']:<12} | {res['time'] / 60.0:<10.2f}",
                log_files=[master_log],
            )
        else:
            all_success = False
            log_print(
                f"{r_name:<12} | FAILED     | {'N/A':<8} | {'N/A':<14} | {'N/A':<12} | {res['time'] / 60.0:<10.2f}",
                log_files=[master_log],
            )
