...
def main():
    ...
    ap.add_argument(
        "--update-rig",
        action="store_true",
        help="overwrite rig.yaml with calibrated extrinsics",
    )
    args = ap.parse_args()
    out = Path(args.out)
    out_cal = out / "cal"
    ...
    for ci in range(4):
        ...
        if has_cv2:
            ...
            cal, pix_y = calibration_from_opencv(
                K, dist.ravel(), rvec0, tvec0, imx=2560, imy=2048, pix_x=0.005
            )
            import scipy.spatial.transform

            Rs = [
                scipy.spatial.transform.Rotation.from_rotvec(r.ravel()).as_matrix()
                for r in rvecs
            ]
    ...
    if not rig_path.exists():
        rig = {
            ...
            "cameras": [
                {
                    "position": [2528, 700, 2528],
                    "target": [0, 615, 0],
                    "focal_mm": 35,
                    "up": [0, 1, 0],
                },
                {
                    "position": [2528, 2900, 2528],
                    "target": [0, 615, 0],
                    "focal_mm": 35,
                    "up": [0, 1, 0],
                },
