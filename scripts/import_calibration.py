...
def _parse_args() -> argparse.Namespace:
    ...
    def _add_import_args(p):
        p.add_argument(
            "--model-dir", type=Path, help="dir with calib_c*.txt (15-float OpenCV)"
        )
        ...
        p.add_argument(
            ...
            type=float,
            required=True,
            help="pixel pitch in mm (pix_x); pix_y derived as cc/fy",
        )
        p.add_argument(
            "--pix-y",
            type=float,
            default=None,
            help="override pix_y when fx!=fy is not isotropic",
        )
        p.add_argument(
            "--out",
            type=Path,
            required=True,
            help="output directory for camN.ori/.addpar",
        )
        p.add_argument(
            "--no-refine",
            action="store_true",
            help="pure algebraic import, no bundle adjust",
        )
        ...
        p.add_argument(
            ...
            type=int,
            default=15,
            help="sortgrid eps for refinement (presorted bypass still needs it for report)",
        )
        p.add_argument(
            "--glass-vec",
            nargs=3,
    ...
    ap_seed.add_argument(
        "--dataset",
        type=Path,
        required=True,
        help="dataset root (for cam_files resolution)",
    )
    ap_seed.add_argument(
        "--out",
        type=Path,
        default=None,
        help="override output dir (default: dataset via cam_files)",
    )
    ap_seed.add_argument("--overwrite", action="store_true")
    ap_seed.add_argument(
        "--dry-run",
        action="store_true",
        help="print derived eps / sortgrid counts without writing",
    )

    args = ap.parse_args()
    # Normalize: if cmd is None but import flags given, treat as import
    if args.cmd is None and (
        getattr(args, "model_dir", None) is not None
        or getattr(args, "points_dir", None) is not None
    ):
        args.cmd = "import"
    if args.cmd is None:
        ap.print_help()
...
def _find_model_files(model_dir: Path, num_cams: int) -> list[Path]:
    ...
    for cam in range(num_cams):
        ...
        for name in [
            ...
            # glob fallback
            matches = sorted(model_dir.glob(f"*c{cam}*calib*.txt")) + sorted(
                model_dir.glob("*calib*.txt")
            )
            if matches:
...
def _find_points_files(points_dir: Path, num_cams: int) -> list[Path]:
    ...
    for cam in range(num_cams):
        for name in [
            f"c{cam}_xyXYZ.txt",
            f"c{cam + 1}_xyXYZ.txt",
            f"cam{cam}_xyXYZ.txt",
        ]:
            p = points_dir / name
            if p.exists():
                out.append(p)
            ...
            if len(matches) >= num_cams:
                ...
                raise FileNotFoundError(
                    f"no points file for cam {cam} in {points_dir} (tried c{{cam}}_xyXYZ.txt)"
                )
    return out
...
def _run_import(args) -> int:
    ...
    cpar = ControlPar(
        ...
        hp_flag=1,
        allCam_flag=0,
        img_base_name=[""] * num_cams,
        cal_img_base_name=[""] * num_cams,
    )
    ...
    for cam, mf in enumerate(model_files):
        ...
        K = np.array(
            [[rec["fx"], 0, rec["cx"]], [0, rec["fy"], rec["cy"]], [0, 0, 1.0]]
        )
        ...
        print(
            f"cam{cam + 1}: seed C=({cal.ext_par.x0:.1f},{cal.ext_par.y0:.1f},{cal.ext_par.z0:.1f}) cc={cal.int_par.cc:.3f} pix_y={pix_y_used:.6f}"
        )
    ...
    for cam in range(num_cams):
        ...
        res = calibrate_from_source(
            "points_file", cam, cpar, ps, eps=eps, presorted=True
        )
        results.append(res)
        ori = out / f"cam{cam + 1}.ori"
        add = out / f"cam{cam + 1}.addpar"
        ...
        print(
            f"cam{cam + 1}: refined matched {res.matched}/{res.nfix} RMS={res.rms:.4f}px flags={'+'.join(res.flags)} -> {ori}"
        )
    ...
    if rcm:
        print(
            f"cross_camera RCM: median={rcm['median']:.4f}mm p90={rcm['p90']:.4f} n={rcm['n_points']} common={rcm['n_common']}"
        )
    else:
...
def _run_seed(args) -> int:
    ...
    if getattr(args, "out", None):
        ...
        print(
            "error: seed --out is not implemented; .ori are written into the "
            "dataset via cam_files",
            file=sys.stderr,
        )
        return 2
    ...
    if dry_run:
        ...
        for cam, cal in cals.items():
            print(
                f"cam{cam + 1}: C=({cal.ext_par.x0:.1f},{cal.ext_par.y0:.1f},{cal.ext_par.z0:.1f}) "
