...
print(f"  p0.5={np.percentile(dx_arr, 0.5):.3f}, p99.5={np.percentile(dx_arr, 99.5):.3f}")
...
print(f"  p0.5={np.percentile(dy_arr, 0.5):.3f}, p99.5={np.percentile(dy_arr, 99.5):.3f}")
...
print(f"  p0.5={np.percentile(dz_arr, 0.5):.3f}, p99.5={np.percentile(dz_arr, 99.5):.3f}")

print("\n3D Displacement Magnitude (mm/frame):")
print(f"  median={np.median(mag_arr):.3f}, p95={np.percentile(mag_arr, 95):.3f}, p99={np.percentile(mag_arr, 99):.3f}, max={mag_arr.max():.3f}")

if len(acc_arr) > 0:
    print("\n3D Acceleration Magnitude dacc (mm/frame^2):")
    print(f"  median={np.median(acc_arr):.3f}, p95={np.percentile(acc_arr, 95):.3f}, p99={np.percentile(acc_arr, 99):.3f}, max={acc_arr.max():.3f}")
...
dacc_rec = float(np.ceil(np.percentile(acc_arr, 99.5) * 10) / 10.0) if len(acc_arr) > 0 else 5.0
