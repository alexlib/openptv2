...
def find_plate_roi(work8: np.ndarray, sigma: float = 25, pad: float = 0.07):
    ...
    for i in range(1, n+1):
        ...
        area=len(xs); x0,x1=int(xs.min()),int(xs.max()); y0,y1=int(ys.min()),int(ys.max())
        areas.append((area,(x0,y0,x1-x0+1,y1-y0+1)))
    areas.sort(reverse=True)
    _,(x,y,w,h)=areas[0]
    x0=int(max(1,x-w*pad)); y0=int(max(1,y-h*pad)); x1=int(min(imx-1,x+w+w*pad)); y1=int(min(imy-1,y+h+h*pad))
    if x1-x0<80 or y1-y0<80:
        return 1,imx-1,1,imy-1,thresh,bw
    return x0,x1,y0,y1,thresh,bw

def _outer_mean(work8,x,y,r_outer=25,r_inner=6):
    xi=int(round(x)); yi=int(round(y)); imy,imx=work8.shape
...
def reject_by_neighbor_cost(cent,target=42,k=5):
    ...
    if len(cent)<=target: return cent
    tree=cKDTree(cent); dists,_=tree.query(cent,k=k); cost=np.sum(dists[:,1:],axis=1)
...
def reject_outside_grid_v2(cent,work8=None,target=42,outer_thresh=100):
    ...
    if work8 is not None and len(filt)==target:
        tree=cKDTree(cent); dists,_=tree.query(cent,k=5); cost=np.sum(dists[:,1:],axis=1); order=np.argsort(cost)
        ...
        if low_mask.any():
            for idx in order[target:]:
                if _outer_mean(work8,cent[idx,0],cent[idx,1])>outer_thresh:
                    worst=int(np.argmin(outer_filt)); filt[worst]=cent[idx]; outer_filt[worst]=_outer_mean(work8,cent[idx,0],cent[idx,1])
                    if (outer_filt<outer_thresh).sum()==0: break
    return filt
...
def detect_plate_points(image_path:Path,pitch_val:float,gv_val:int,sumg_val:int):
    ...
    if raw.ndim==3: raw=np.mean(raw,axis=2).astype(raw.dtype)
    ...
    cpar=ControlPar(num_cams=1,imx=2560,imy=2048,pix_x=0.005,pix_y=0.005,mm=MmNp(n1=1,n2=[1],d=[0],n3=1),chfield=0,tiff_flag=1,hp_flag=1,allCam_flag=0,img_base_name=[""],cal_img_base_name=[""])
    ...
    try:
        ...
        if found:
            corners=corners.reshape(-1,2); corners[:,0]+=xmin; corners[:,1]+=ymin; cv2_corners=corners
    except Exception: pass
    return {"raw_path":image_path,"work8":work8,"roi":(xmin,xmax,ymin,ymax),"cent_raw":np.array([[t.x,t.y] for t in tg],float) if tg else np.zeros((0,2)),"cent_filt":cent,"n_raw":n_raw,"n_filt":len(cent),"cv2_corners":cv2_corners}
...
def main():
    ...
    args=ap.parse_args()
    base=Path(args.base); out=Path(args.out); pitch_val=float(args.pitch); gv_val=int(args.gv); sumg_val=int(args.sumg)
    ...
    for ci,fld in enumerate(folders):
        ...
        for p in sorted([x for x in tifs if x.name[:8].isdigit()]):
            frame=p.name.split("_")[0]; groups[frame][ci]=p
    ...
    for frame in sync_frames:
        for ci in range(4):
            ...
            try:
                ...
                print(f"  {frame} cam{ci+1} label fail {e}"); img_pts,ref_pts=cent,np.zeros((0,3))
            # store per-frame
            per_cam_frames[ci].append({"frame":frame,"path":str(path),"roi":det["roi"],"n_raw":det["n_raw"],"n_filt":len(cent),"n_labeled":len(img_pts),"img_pts":img_pts,"ref_pts":ref_pts,"cv2_corners":det["cv2_corners"]})
            if len(img_pts)>=20:
                flat_collections[ci].append((ref_pts,img_pts,frame))
            print(f"  {frame} cam{ci+1}: {det['n_raw']}->{len(cent)}->{len(img_pts)} roi {det['roi'][:2]}")
    # save
    out_cal=out/"cal"; out_cal.mkdir(parents=True,exist_ok=True)
    for ci in range(4):
        # per-camera npz with 3D-2D pairs
        arr=np.array(per_cam_frames[ci],dtype=object)
        # also flat for cal
        np.savez(out_cal/f"detections_cam{ci+1}.npz",
                 frames=np.array([x["frame"] for x in per_cam_frames[ci]]),
                 n_raw=np.array([x["n_raw"] for x in per_cam_frames[ci]]),
                 n_filt=np.array([len(reject_by_neighbor_cost(np.array([[0,0]])))]), # placeholder
                 )
        # save explicit 3D-2D pairs as separate npz for Part 2
        # flat_collections already has ref/img
        np.savez(out_cal/f"pairs_cam{ci+1}.npz",
    ...
    with open(out_cal/"detections_summary.txt","w") as fh:
        fh.write(f"pitch {pitch_val} gv {gv_val} sumg {sumg_val} sync_frames {len(sync_frames)}\n")
        for ci in range(4):
            fh.write(f"cam{ci+1}: {len(flat_collections[ci])} frames kept, {sum(len(x[0]) for x in flat_collections[ci])} points total\n")
            for fr,img,ref in [(f,img,ref) for ref,img,f in flat_collections[ci][:3]]:
                fh.write(f"  e.g. {fr}: {len(ref)} pts\n")
    print(f"\nPart 1 done -> {out_cal}")
    for ci in range(4):
        print(f" cam{ci+1}: {len(flat_collections[ci])} frames, {sum(len(x[0]) for x in flat_collections[ci])} points -> pairs_cam{ci+1}.npz + detections_cam{ci+1}.npz")
    print("Next: Part 2 -> uv run --with opencv-python python scripts/calibrate_illmenau_4cam_part2.py")
