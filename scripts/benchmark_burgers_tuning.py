...
def phys(yaml_path, tracker, ov):
    ...
    try:
        ...
        return {
            ...
            "kurt": round(float(pm.acceleration_kurtosis), 1)
            if np.isfinite(pm.acceleration_kurtosis)
            else None,
            "pred": pred,
        }
...
def sweep(yaml_path, tracker, base_dvx, base_dacc, base_ang):
    ...
    dacc_vals = sorted(
        set([max(0.1, base_dacc * 0.5), base_dacc, min(10, base_dacc * 3)])
    )
    ...
    for dv in dv_vals:
        for da in dacc_vals:
            for ang in ang_vals:
                ...
                rows.append(r)
                if r["error"] is None:
                    score = r["mean_len"] - 0.02 * (
                        r["kurt"] if r["kurt"] is not None else 20
                    )
...
def run_dataset(label, yaml_path, base_dvx, base_dacc, base_ang):
    ...
    for tr in ALL_TRACKERS:
        ...
        if best is None:
            ...
            continue
        print(
            f"{tr:25} dv={best['dvxmax']:4.1f} dacc={best['dacc']:4.1f} ang={best['angle']:3.0f}  mean_len={best['mean_len']:4.1f} frac10={best['frac10']:.2f} kurt={best['kurt']} time={best['time_s']}s  {best['error'] or ''}"
        )
        all_rows.append(
            {
                "tracker": tr,
...
if __name__ == "__main__":
    ...
    df_b = run_dataset(
        "burgers (5 fr, vortex)", burgers, base_dvx=2.0, base_dacc=0.5, base_ang=60
    )
    print(df_b.to_string(index=False))
    df_t = run_dataset(
        "synthetic_turbulent_1k (10 fr, 1k)",
        turb,
        base_dvx=8.0,
        base_dacc=5.0,
        base_ang=120,
