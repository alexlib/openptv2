...
"""Turnkey headless calibration CLI for an OpenPTV dataset.
    ...
    uv run python scripts/autocalibrate.py test_data/test_cavity --dry-run --no-overlays
...
def main() -> int:
    ...
    ap.add_argument("--dry-run", action="store_true",
                    help="do not overwrite .ori/.addpar")
    ...
    for r in results:
        print(f"cam{r.cam + 1}: matched {r.matched}/{r.nfix}  RMS={r.rms:6.3f}px  "
