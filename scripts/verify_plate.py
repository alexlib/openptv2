...
def _load_cals(cals_dir: Path, num_cams: int, pix: float):
    ...
    for cam in range(num_cams):
        ...
        add = ori.with_suffix(".addpar") if not str(ori).endswith(".ori") else Path(str(ori).replace(".ori", ".addpar"))
...
def main() -> int:
    ...
    ap.add_argument("--cals", type=Path, required=True, help="dir with camN.ori/.addpar")
    ap.add_argument("--points-dir", type=Path, default=None, help="dir with c*_xyXYZ.txt for triangulation check")
    ...
    ap.add_argument("--pitch", type=float, default=40.0, help="expected grid pitch in mm")
    args = ap.parse_args()

    cals = _load_cals(args.cals, args.num_cams, args.pix)
    cpar = ControlPar(num_cams=args.num_cams, imx=args.imx, imy=args.imy, pix_x=args.pix, pix_y=args.pix,
                      mm=MmNp(n1=1.0, n2=[1.0], d=[0.0], n3=1.0),
                      chfield=0, tiff_flag=1, hp_flag=1, allCam_flag=0,
                      img_base_name=[""]*args.num_cams, cal_img_base_name=[""]*args.num_cams)
    ...
    if args.points_dir is not None:
        ...
        for cam in range(args.num_cams):
            # Find points file
            candidates = list(args.points_dir.glob(f"c{cam}_xyXYZ.txt")) + list(args.points_dir.glob(f"c{cam+1}_xyXYZ.txt")) + list(args.points_dir.glob("*xyXYZ*.txt"))
            if not candidates:
                continue
            ...
            rep = np.array([_reproject_px(cals[cam], cpar.mm, r, cpar) for r in ref_pts])
            rms = rms_px(img_pts, rep)
            print(f"cam{cam+1}: {len(ref_pts)} pts  RMS={rms:.4f}px  (file {pfile.name})")
            ...
            for z in uniq[:3]:
                ...
                print(f"  Z={z:g}  n={len(plane)}  planarity RMS={float(np.sqrt(np.mean(dists**2))):.4f}mm max={float(dists.max()):.4f}mm")
                # Pitch check: nearest neighbour distances
                from scipy.spatial import cKDTree
                tree = cKDTree(plane[:, :2])
                d, _ = tree.query(plane[:, :2], k=2)
                nn = d[:, 1]
                print(f"    pitch NN median={float(np.median(nn)):.2f} (expected {args.pitch}) std={float(nn.std()):.2f}")
    ...
    if args.points_dir is not None:
        ...
        for cam in range(args.num_cams):
            ...
            rep = np.array([_reproject_px(cals[cam], cpar.mm, r, cpar) for r in ref_pts])
            rms = rms_px(img_pts, rep)
            results.append(CamResult(cam=cam, matched=len(ref_pts), nfix=len(ref_pts), rms=rms, flags=[], cal=cals[cam], ref=ref_pts, det=img_pts, rep=rep))
        if len(results) >= 2:
            rcm = cross_camera_rcm(results, cpar)
            if rcm:
                print(f"cross_camera RCM: median={rcm['median']:.4f}mm p90={rcm['p90']:.4f} max={rcm['max']:.4f} n={rcm['n_points']}")
            else:
